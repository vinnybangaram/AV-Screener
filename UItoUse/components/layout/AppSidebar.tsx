...
import { Sparkles, LifeBuoy, Crown, TrendingUp } from "lucide-react";
import {
  ...
  return (
    <Sidebar collapsible="icon" className="border-sidebar-border [&>div]:!bg-transparent">
      ...
      <SidebarFooter className="relative border-t border-sidebar-border/60 p-3 gap-2 z-10">
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton tooltip="Help Center" className="h-9 text-sidebar-foreground hover:bg-sidebar-accent/60">
        ...
        <div className="flex items-center gap-2.5 rounded-lg bg-sidebar-accent/40 p-2 border border-sidebar-border/60">
          ...
          {!collapsed && (
            <div className="flex flex-col leading-tight min-w-0">
              <span className="text-xs font-semibold text-sidebar-primary-foreground truncate">Arjun Verma</span>
              <span className="text-[10px] text-sidebar-foreground/70 truncate">Pro Trial · 12 days</span>
            </div>
          )}
        </div>
      </SidebarFooter>
