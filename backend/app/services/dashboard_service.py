...
def get_dashboard_data(db: Session, user_id: int, category: str = "All", timeframe: str = "This Month") -> Dict[str, Any]:
    ...
    if category != "All":
        cat_map = {"Penny Stocks": "Penny", "Multibaggers": "Multibagger", "Intraday Radar": "Intraday"}
    ...
    # Separate positions into groups (using filtered timeframe list for metrics)
    investment_positions_all = [item for item in filtered_timeframe if item["category"].lower() in ("multibagger", "penny")]
    intraday_positions_all   = [item for item in filtered_timeframe if item["category"].lower() == "intraday"]
    ...
    def calculate_group_metrics(positions):
        ...
        total_value = sum(item["latest_price"] * item["quantity"] for item in positions if item["is_active"])
        ...
        long_pl_abs = sum(item["latest_pnl"] * item["quantity"] for item in positions if item["side"] != "SHORT")
        short_pl_abs = sum(item["latest_pnl"] * item["quantity"] for item in positions if item["side"] == "SHORT")
        ...
        active_symbols = [item["symbol"] for item in positions if item["is_active"]]
        ...
        return {
            ...
            "count": len([p for p in positions if p["is_active"]]) # User wants active count
        }

    investment_metrics = calculate_group_metrics(investment_positions_all)
    intraday_metrics   = calculate_group_metrics(intraday_positions_all)

    # Calculate Last 30 Days PnL (Legacy support)
